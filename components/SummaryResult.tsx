...
export function SummaryResult({ result }: { result: SummarizeResponse }) {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      await downloadSummaryPdf(result, theme);
    } finally {
  ...
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--paper-card)] p-5 sm:p-8">
      ...
      {result.ocrPageInfo && result.ocrPageInfo.pagesProcessed < result.ocrPageInfo.totalPages && (
        <p className="text-xs text-[var(--muted)] mb-4">
          OCR processed the first {result.ocrPageInfo.pagesProcessed} of {result.ocrPageInfo.totalPages} pages
          in this scanned PDF. The summary below is based only on those pages.
        </p>
      )}
        ...
        <div className="mt-8">
          ...
          <ul className="space-y-2.5">
            {result.improvementSuggestions.map((suggestion, i) => (
              <li
                key={i}
                className="flex gap-3 items-start text-sm sm:text-base text-[var(--ink-soft)] leading-relaxed"
              >
                <span className="mt-2 h-1.5 w-1.5 rounded-full bg-[var(--highlighter-dark)] shrink-0" />
