...
// count, we treat the PDF as "likely scanned" and the caller renders
// its pages to images and runs them through OCR instead (see
// renderPdfPagesToImages below).
...
// Rendering a scanned PDF's pages to images so OCR can run on them.
//
// This uses pdf-parse's getScreenshot(), which rasterizes each page via
// pdfjs-dist — the same underlying engine getText() above uses, and it
// needs the same CanvasFactory polyfill for the same reason (no native
// browser canvas in Node). Since that polyfill is already wired up for
// text extraction, rendering full page images is "free" in terms of new
// infrastructure: no extra native dependency, no Poppler binary, nothing
// that would be fragile on Vercel — @napi-rs/canvas ships prebuilt
// binaries for Vercel's Linux x64 runtime.
//
// OCR-ing a whole PDF page-by-page is inherently slower than OCR-ing a
// single uploaded image, so we cap how many pages we'll render per
...
// scale 2.5 on a PDF's native 72-DPI coordinate space works out to
// roughly 180 DPI, comfortably inside Tesseract's recommended ~150-300
// DPI range for accurate recognition, without producing images large
// enough to meaningfully slow down OCR or bloat memory use.
const SCREENSHOT_SCALE = 2.5;
...
export interface PdfPageImagesResult {
  images: PdfPageImage[];
  totalPages: number;
  /** true if the PDF has more pages than we rendered, i.e. MAX_SCANNED_PDF_PAGES was hit. */
  truncated: boolean;
}
...
export async function renderPdfPagesToImages(
  ...
  try {
    ...
    const images: PdfPageImage[] = result.pages.map((page, index) => ({
      pageNumber: index + 1,
      data: Buffer.isBuffer(page.data) ? page.data : Buffer.from(page.data),
    }));
