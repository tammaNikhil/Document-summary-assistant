...
import { extractPdfText, renderPdfPagesToImages } from "@/lib/extract-pdf";
...
// Scanned PDFs are rendered page-by-page and OCR'd (see the
// "likelyScanned" branch below), which is slow enough that the old 60s
// budget wasn't enough headroom for anything but a single page. 300s is
// the maximum function duration Vercel's Hobby plan allows without
// opting into Fluid Compute's higher tiers, so it's the largest budget
// available without asking anyone deploying this to upgrade.
export const maxDuration = 300;

// How much of the request's total time budget the OCR loop itself is
// allowed to consume, once a scanned PDF's pages have been rendered to
// images. This leaves the remaining ~90s of the 300s budget for
// upstream work (reading the upload, the initial text-extraction pass,
// rendering pages to images) and downstream work (the summarization
// call), so a many-page scanned PDF degrades to "OCR'd as much as we
// had time for" instead of blowing the whole request's timeout.
      ...
      try {
        ...
        if (file.type === "application/pdf") {
          ...
          if (pdfResult.likelyScanned) {
            ...
            let pageImages;
            try {
              pageImages = await renderPdfPagesToImages(buffer, pdfResult.numPages);
            } catch (renderErr) {
              console.error("PDF page rendering error:", renderErr);
              send({
                stage: "error",
                error:
                  "This PDF appears to be scanned, but its pages couldn't be rendered for OCR. " +
